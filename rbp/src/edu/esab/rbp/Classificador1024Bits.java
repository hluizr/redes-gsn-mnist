...
/**
 * Classificador para padroes de 256 entradas.
 *
 * @author Helio Rodrigues
 */
public class Classificador1024Bits implements Classificador {

    public static final int TAMANHO_ENTRADA = 1024;

    private final Classificador512Bits[] unidadeEntrada = new Classificador512Bits[2];
    private final Classificador2Bits unidadeSaida;

    /**
     * Inicializa a unidade logica.
     */
    public Classificador1024Bits() {
        for (int i = 0; i < unidadeEntrada.length; i++) {
            unidadeEntrada[i] = new Classificador512Bits();
        }
