...
/**
 * Classificador para padroes de 16 entradas.
 *
 * @author Helio Rodrigues
 */
public class Classificador32Bits implements Classificador {

    public static final int TAMANHO_ENTRADA = 32;

    private final Classificador16Bits[] unidadeEntrada = new Classificador16Bits[2];
    private final Classificador2Bits unidadeSaida;

    /**
     * Inicializa a unidade logica.
     */
    public Classificador32Bits() {
        for (int i = 0; i < unidadeEntrada.length; i++) {
            unidadeEntrada[i] = new Classificador16Bits();
        }
