...
import edu.esab.rbp.Classificador8Bits;
 ...
 */
public class ClassificadorMnist1024x128xSomador implements ClassificadorMnist {
    
    private final Classificador8Bits[][] classificadores = new Classificador8Bits[5][128];
    private final int[][] classes = new int[10][128];
     ...
     * Constroi a rede.
     */
    public ClassificadorMnist1024x128xSomador() {
        // Piramide de classificacao.
        for (int i = 0; i < classificadores.length; i++) {
            for (int j = 0; j < classificadores[i].length; j++) {
                classificadores[i][j] = new Classificador8Bits();
            }
        }

        // Vetores de classes.
        for (int i = 0; i < 5; i++) {
            classes[i] = Utilidades.criarVetorAleatorio(128);
        }
    ...
    public void calibrar(ImagemMnist imagemMnist, boolean adivinhar) {
        int[] imagem = Utilidades.converterImagemEntrada(imagemMnist.getImagem());
        int[][] classificacao = new int[5][128];
