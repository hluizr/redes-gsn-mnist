...
/**
 * Arquitetura GSN com 1024 bits de entrada e 64 bits de saida.
 *
 * @author Helio Rodrigues.
 */
public class ClassificadorMnist1024x64xSomador implements ClassificadorMnist {

    private final Classificador16Bits[][] classificadores = new Classificador16Bits[5][64];
    private final int[][] classes = new int[10][64];
     ...
     * Constroi a rede.
     */
    public ClassificadorMnist1024x64xSomador() {
        // Piramide de classificacao.
        for (int i = 0; i < classificadores.length; i++) {
            for (int j = 0; j < classificadores[i].length; j++) {
                classificadores[i][j] = new Classificador16Bits();
            }
        }

        // Vetores de classes.
        for (int i = 0; i < 5; i++) {
            classes[i] = Utilidades.criarVetorAleatorio(64);
        }
    ...
    public void calibrar(ImagemMnist imagemMnist, boolean adivinhar) {
        int[] imagem = Utilidades.converterImagemEntrada(imagemMnist.getImagem());
        int[][] classificacao = new int[5][64];
