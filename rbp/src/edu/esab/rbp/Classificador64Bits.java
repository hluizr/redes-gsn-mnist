...
/**
 * Classificador para padroes de 64 entradas.
 *
 * @author Helio Rodrigues
 */
public class Classificador64Bits implements Classificador {

    public static final int TAMANHO_ENTRADA = 64;

    private final Classificador32Bits[] unidadeEntrada = new Classificador32Bits[2];
    private final Classificador2Bits unidadeSaida;

    /**
     * Inicializa a unidade logica.
     */
    public Classificador64Bits() {
        for (int i = 0; i < unidadeEntrada.length; i++) {
            unidadeEntrada[i] = new Classificador32Bits();
        }
