...
/**
 * Classificador para padroes de 8 entradas.
 *
 * @author Helio Rodrigues
 */
public class Classificador8Bits implements Classificador {

    public static final int TAMANHO_ENTRADA = 8;

    private final Classificador4Bits[] unidadeEntrada = new Classificador4Bits[2];
    private final Classificador2Bits unidadeSaida;

    /**
     * Inicializa a unidade logica.
     */
    public Classificador8Bits() {
        for (int i = 0; i < unidadeEntrada.length; i++) {
            unidadeEntrada[i] = new Classificador4Bits();
        }
